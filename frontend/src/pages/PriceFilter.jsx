...
const PriceFilter = ({ addToCart }) => {
  const [foods, setFoods] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  const [priceRange, setPriceRange] = useState('All'); // All, Under $5, $5 - $10, $10 - $15, Above $15
  ...
  useEffect(() => {
    const fetchFoods = async () => {
      try {
        ...
        console.error("Error fetching foods for price filter", error);
        setLoading(false);
      }
    };
    fetchFoods();
  ...
  let filteredFoods = [...foods].filter(food => {
    ...
    if (priceRange === 'Under $5') {
      matchesPrice = food.price < 5;
    } else if (priceRange === '$5 - $10') {
      matchesPrice = food.price >= 5 && food.price <= 10;
    } else if (priceRange === '$10 - $15') {
      matchesPrice = food.price > 10 && food.price <= 15;
    } else if (priceRange === 'Above $15') {
      matchesPrice = food.price > 15;
    }
    ...
    <div className="menu-container">
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '3rem' }}>
          <h3>Loading advanced filters...</h3>
        </div>
      ) : (
        <div className="food-grid">
          {filteredFoods.length > 0 ? (
            ...
            <div className="no-results" style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '2rem' }}>
              <p>No food items found matching your filter criteria.</p>
              <button onClick={() => { setPriceRange('All'); setSearchTerm(''); setSortOrder('None'); }} className="btn-primary" style={{ marginTop: '1rem' }}>
                Clear Filters
              </button>
            </div>
          )}
